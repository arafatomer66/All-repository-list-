...
class ReposArea extends Component {
  ...
  getData = async() => {
    try {
      const { data: repos1 } = await axios(`https://api.github.com/users/arafatomer66/repos?page=1&sort=created&type=owner`);
      const { data: repos2 } = await axios(`https://api.github.com/users/arafatomer66/repos?page=2&sort=created&type=owner`);
